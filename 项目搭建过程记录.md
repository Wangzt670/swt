...
![image-20220624145447444](http://wkf-picture.oss-cn-beijing.aliyuncs.com/img/image-20220624145447444.png)
...
地址簿，指的是移动端消费者用户的地址信息，用户登录成功后可以维护自己的地址信息。同一个用户可以有多个地址信息，但是只能由一个**默认地址**。
...
用户登录成功后跳转到系统首页，在首页需要根据费雷来展示菜品和套餐。如果菜品设置了口味信息，需要展示**选择规格**按钮，否则显示**+**按钮。

#### 交互过程

1. 点击**加入购物车**或者**+**按钮,页面发送ajax请求,请求服务端,将菜品或者套餐添加到购物车
2. 点击购物车图标,页面发送ajax请求,请求服务端查询购物车中得菜品和套餐
3. 点击清空购物车按钮,页面发送ajax请求,请求服务端来执行清空购物车操作.

开发购物车功能,在服务端编写代码去处理前端页面发送得这3次请求.
...
移动端用户可以将菜品或者套餐添加到购物车。对于菜品来说，如果设置了口味信息，这需要选择规格后才能加入购物车；对于套餐来说，可以直接点击**+**将当前套餐加入购物车。在购物车中可以修改菜品和套餐的数量，也可以清空购物车。
...
4. 在订单确认页面单机**去支付**按钮,发送ajax请求,请求服务端完成下单操作.
...
![image-20220624144315933](http://wkf-picture.oss-cn-beijing.aliyuncs.com/img/image-20220624144315933.png)

## 缓存菜品数据

### 实现思路

![image-20220624144601074](http://wkf-picture.oss-cn-beijing.aliyuncs.com/img/image-20220624144601074.png)

![image-20220625150705043](http://wkf-picture.oss-cn-beijing.aliyuncs.com/img/image-20220625150705043.png)
